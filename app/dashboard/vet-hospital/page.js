...
import { IoLocationOutline, IoTimerOutline, IoEarthSharp } from 'react-icons/io5';
  ...
  return (
    ...
    <div className='w-full bg-gray-400 p-80 h-40 ml-12 '>
      ...
      {items.map((item, index) => (
        <div key={index} className={`flex flex-col shadow shadow-gray-400 p-8 w-80 rounded-xl ${index === currentIndex || index === currentIndex + 1 || index === currentIndex + 2 ? '' : 'hidden'}`}>
      ...
      <div className=''>
         <FaChevronLeft className="w-10 h-10  absolute top-1/2 left-0 transform -translate-y-1/2 bg-gray-300 p-2 rounded-full" onClick={prevSlide}></FaChevronLeft>
      <FaChevronRight className="w-10 h-10 absolute top-1/2 right-0 transform -translate-y-1/2 bg-gray-300 p-2 rounded-full" onClick={nextSlide}>Next</FaChevronRight>
      </div>
